...
if(arg(0) ==  "node"){
	...
	$str = file_get_contents($node->field_drupal_module_link['und'][0]['value'].'/release/feed');
	 ...
	 if($str){
		...
		$drupal_download='<div class="drupal-download" style="display: inline; width: 42%; float: left;"><h4>Recommended releases</h4>'.htmlspecialchars_decode($cut_str2).'</div>
			 ...
			 <div class="see-all-releases"><h4 style="text-decoration: underline; color: rgb(66, 139, 202); text-align: right; width: auto; height: auto; overflow: hidden; position: relative; top: -10px;">'.l('See all releases', $node->field_drupal_module_link['und'][0]['value'].'/release', array('attributes'=>array('target'=>'_blank'))).'</h4></div><hr>';
